...
use crate::error::{Error, FontInfoError, FontInfoErrorKind};
...
impl FontInfo {
    ...
    pub(crate) fn from_file<P: AsRef<Path>>(
        ...
        lib: &mut Plist,
    ) -> Result<Self, FontInfoError> {
        let path = path.as_ref();
        match format_version {
            FormatVersion::V3 => {
                let mut fontinfo: FontInfo =
                    plist::from_file(path).map_err(FontInfoError::ParsingFontInfoFile)?;
                fontinfo.validate().map_err(FontInfoError::InvalidData)?;
            ...
            FormatVersion::V2 => {
                ...
                fontinfo.validate().map_err(FontInfoError::FontInfoUpconversion)?;
                Ok(fontinfo)
            }
            FormatVersion::V1 => {
                ...
                let fontinfo = FontInfo {
                    ...
                    open_type_os2_width_class: match fontinfo_v1.widthName {
                        Some(v) => match v.as_ref() {
                            ...
                            _ => {
                                return Err(FontInfoError::FontInfoUpconversion(
    ...
    pub fn validate(&self) -> Result<(), FontInfoErrorKind> {
        ...
        if let Some(v) = &self.postscript_family_other_blues {
            if v.len() > 10 {
                return Err(FontInfoErrorKind::InvalidPostscriptListLength(
                    "postscriptFamilyOtherBlues".into(),
                    10,
                    v.len(),
                ));
            }
        ...
        if let Some(v) = &self.woff_metadata_extensions {
            if v.is_empty() {
                return Err(FontInfoErrorKind::EmptyWoffAttribute("woffMetadataExtensions".into()));
            }

            for record in v.iter() {
                if record.items.is_empty() {
                    return Err(FontInfoErrorKind::EmptyWoffAttribute(
                        "woffMetadataExtensions record, items".into(),
                    ));
                }
                ...
                for record_item in record.items.iter() {
                    if record_item.names.is_empty() || record_item.values.is_empty() {
                        return Err(FontInfoErrorKind::EmptyWoffAttribute(
                            "woffMetadataExtensions record, item names or values".into(),
                        ));
                    }
                }
            }
        ...
        if let Some(v) = &self.woff_metadata_credits {
            if v.credits.is_empty() {
                return Err(FontInfoErrorKind::EmptyWoffAttribute("woffMetadataCredits".into()));
            }
        }
        if let Some(v) = &self.woff_metadata_copyright {
            if v.text.is_empty() {
                return Err(FontInfoErrorKind::EmptyWoffAttribute("woffMetadataCopyright".into()));
            }
        }
        if let Some(v) = &self.woff_metadata_description {
            if v.text.is_empty() {
                return Err(FontInfoErrorKind::EmptyWoffAttribute(
                    "woffMetadataDescription, text".into(),
                ));
            }
        }
        if let Some(v) = &self.woff_metadata_trademark {
            if v.text.is_empty() {
                return Err(FontInfoErrorKind::EmptyWoffAttribute("woffMetadataTrademark".into()));
            }
        }
    ...
    fn load_object_libs(&mut self, lib: &mut Plist) -> Result<(), FontInfoError> {
        let mut object_libs = match lib.remove(PUBLIC_OBJECT_LIBS_KEY) {
            Some(lib) => {
                lib.into_dictionary().ok_or(FontInfoError::PublicObjectLibsMustBeDictionary)?
            }
            None => return Ok(()),
        };
                ...
                if let Some(id) = guideline.identifier() {
                    if let Some(lib) = object_libs.remove(id.as_str()) {
                        let lib = lib.into_dictionary().ok_or_else(|| {
                            FontInfoError::GlobalGuidelineLibMustBeDictionary(
                                id.as_str().to_string(),
                            )
                        })?;
